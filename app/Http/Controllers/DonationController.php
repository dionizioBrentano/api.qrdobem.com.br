 ...
 *
 * ENDPOINTS
 *   POST /donations/preview      (público) detalha o rateio sem gravar nada
 *   POST /donations              (auth opcional) cria a doação e o checkout
 *   GET  /donations/mine         doações do usuário (exige conta)
 *   POST /donations/subscribe    assinatura recorrente (exige conta)
...
class DonationController extends Controller
{
    ...
    /**
     * POST /donations/preview  — PÚBLICO
     *
    ...
    /**
     * POST /donations  — auth OPCIONAL (doar não exige conta)
     *
    ...
    public static function markAsPaid(Donation $donation, ?string $mpStatus = null): void
    {
        ...
        if ($donation->donor_email) {
            try {
                ...
                Log::warning('DonationController: falha ao enviar recibo da doação', [
                    'donation_id' => $donation->id,
                    'error'       => $e->getMessage(),
                ]);
            }
